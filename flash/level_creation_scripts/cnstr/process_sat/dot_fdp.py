...
def layout_with_sfdp(dot_filename, Gs):
    print 'Running fdp -y -Tplain -o%s.out %s ...' % (dot_filename, dot_filename)
    with os.popen('fdp -y -Tplain -o%s.out %s' % (dot_filename, dot_filename)) as fdpcmd:
...
def run(infile, outfile, skip_if_trivial, node_min, node_max, show_labels):
    ...
    header = '''
digraph G {
  graph [ overlap="3:scalexy" K="4"]
  node [ shape="circle" width="3" height="3" label="" %s ]
  edge [ len="1.2" ]
    ''' % label_txt
